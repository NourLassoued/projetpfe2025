...
export class GererdemandeComponent {
    ...
    this.demandeservice.getDemandeById(id).subscribe({
      next: (data) => {
        ...
        if (this.demande?.servicee?.idservice) {
          this.loadPrestatairesCompatibles(this.demande.idDemande!);
        }
  ...
  private loadPrestatairesCompatibles(idDemande: number): void {
    this.utilisateurService.getPrestatairesCompatibles(idDemande).subscribe({
      next: (utilisateurs: any[]) => {
        this.utilisateurs = utilisateurs;

        this.utilisateurs.forEach((utilisateur, index) => {
          this.prepareUtilisateur(utilisateur, index);
        });
      },
      error: (error) => {
  ...
  private prepareUtilisateur(utilisateur: any, index: number): void {
    utilisateur.servicesOfferts = Array.isArray(utilisateur['services'])
      ? utilisateur['services'].map((service: string) => ({
        idservice: undefined,
    ...
    if (utilisateur.idUtilisateur != null) {
      this.loadAvisPourUtilisateur(utilisateur.idUtilisateur);
    } else {
      console.error('idUtilisateur est undefined pour l\'utilisateur:', utilisateur);
    }
  }

  private loadAvisPourUtilisateur(idUtilisateur: number): void {
    this.avisService.getScoreMoyen(idUtilisateur).subscribe({
      next: (score) => {
    ...
    this.avisService.getNombreAvisPourUtilisateur(idUtilisateur).subscribe({
      ...
      error: (error) => {
        console.error(`Erreur lors de la récupération du nombre d'avis pour l'utilisateur ${idUtilisateur} :`, error);
      }
    });
  }
