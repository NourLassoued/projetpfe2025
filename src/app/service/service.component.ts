...
export class ServiceComponent {
    ...
    this.service.getAllServices().subscribe({
      next: (data: Servicee[]) => {
        ...
        this.totalPages = Math.ceil(
          this.filteredservice.length / this.pageSize
        );
        this.currentPage = 1;
  ...
  deleteservice(idservice: number): void {
    if (idservice) {
      this.service.deleteService(idservice).subscribe({
        next: () => {
          this.servicee = this.servicee.filter(
        ...
        error: (err) => {
          ...
          this.toastr.warning(
            "Le service n'a pas pu être supprimé, ID manquant.",
            'Avertissement'
          );
        },
      });
  ...
  updateCategory(): void {
    ...
    if (
      this.selectedservice.tarif !== undefined &&
      this.selectedservice.tarif !== null
    ) {
    ...
    this.service
      .updateService(this.selectedservice.idservice, formData)
      .subscribe(
        (response) => {
          if (this.selectedFile && response.imageService) {
            const index = this.servicee.findIndex(
              (s) => s.idservice === response.idservice
            );
            if (index !== -1) {
